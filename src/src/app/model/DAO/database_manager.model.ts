...
import { permanent_posm_res_partner_rel } from 'src/app/model/data/permanent_posm_res_partner_rel.model';
import { res_users_res_partner_rel } from 'src/app/model/data/res_users_res_partner_rel.model';
...
export class Database_manager {
    ...
    set_visit_duration_end_datetime(visit_sheet_id: number, duration: any, end_datetime: any) {
        return this.init_database()
            .then((db: SQLiteObject) => {
                let query = " update visit_sheet set visit_duration = ? , end_datetime = ? where id = ? ";
                let params = [visit_sheet_id, duration, end_datetime];
    ...
    async start_visit(visit_sheet_id: number) {
        ...
        try {
            ...
            let query = " update visit_sheet set begin_datetime = ? where id = ? ";
            let params = [this.getDatetimeNow(), visit_sheet_id];
            db.executeSql(query, params)
                .then((response) => {
                    db.executeSql("select begin_datetime from visit_sheet where id = ?", [visit_sheet_id])
                        .then((data) => {
                            for (var i = 0; i < data.rows.length; i++) {
                ...
                .then(data => {
                    if (data.rows.length > 0) {
                        for (var i = 0; i < data.rows.length; i++) {
                            ...
                            else data.rows.item(i).state = 'ouvert';
                            data_return.push(data.rows.item(i));
                        }
                        return data_return;
                    }
    ...
    get_data_for_p5(visit_sheet_id: number) {
        return this.init_database().then((db: SQLiteObject) => {
            let sql_select: string = "select itg_product.id as produit_id, itg_product.name as product_name, plv_line.quantity as quantity, plv_line.* from plv_line inner join itg_product on plv_line.product_id = itg_product.id where plv_line.visit_sheet_id = ?";
            let data_return = [];
            return db.executeSql(sql_select, [visit_sheet_id])
                .then(data => {
